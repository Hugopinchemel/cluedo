...
export function useWindows() {
  ...
  function toggleMaximize(id: string) {
    ...
    if (!w) return
    if (!w.maximized) {
      w._prevX = w.x; w._prevY = w.y; w._prevW = w.width; w._prevH = w.height
      w.maximized = true
    } else {
      w.maximized = false
      w.x = w._prevX ?? 80; w.y = w._prevY ?? 50
      w.width = w._prevW ?? 800; w.height = w._prevH ?? 500
    }
    focusWindow(id)
  }
